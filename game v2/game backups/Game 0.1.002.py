...
def team_target(c, l=0):        #used for choosing target
...
def revival(hp, c=0):  #hp for health after revivalion, c=0 for ally ress c=1 for enem ress
    global t_target
    while True:
        print('Who do you want to revival?')
        y = -1    #for indexes
        ...
        if not z:
            print(x.name + ' tries to find anyone that may be in dead need, but he just loses time(and turn)')
            t_target = None
            break
        ...
        if o in z:
            ...
            print(target[o].name + ' has been revivaled and has ' + str(target[o].hp) + ' health points')
            t_target = target[o]
            break
        else:
...
#--------------------------Characters-------------------------------
Hero = Char('Hero', 70, 10, 1, 100, defence=1, weapon=sword)        #name maxhp atk atk_var recruitment_cost=0 g_drop=0 defence=0 defence_type='none' weapon=basic armor='basic'
