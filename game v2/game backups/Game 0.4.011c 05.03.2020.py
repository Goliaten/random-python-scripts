                    ...
                    #---------complete (remind of) a comission------------
                        ...
                        while True:
                            ...
                            if a == 1:
                                ...
                                temp_menu = ['Current comission:'] + cmsn_info(accept_cmsn, 1)
                                sec_menu = []
                                ...
                                if accept_cmsn.type == 'hunt':
                                    ...
                                    z = 1
                                    #---------------------------
                                    kill_counter[1], r_list = list_substr(kill_counter[1], cmsn_kill_counter[1])  #substracting kill counters
                                    for x in r_list[::-1]:
                                        kill_counter[0].pop(x)     #removing IDs, which have no counter in second half of kill_counter
                                    #---------------------------
                                    if kill_counter[1]:
                                        out = 'After last progress report you have slain '
                                        for char in global_char:
                                            if char.ID in [x.ID for x in accept_cmsn.prog_kill]:
                                                ind = [x.ID for x in accept_cmsn.prog_kill].index(char.ID)
                                                count = accept_cmsn.prog_kill_cnt[ind]
                                    ...
                                    pause()
                                    #---------------------------
                                    accept_cmsn.prog_kill_cnt, r_list = list_substr(accept_cmsn.prog_kill_cnt, kill_counter[1])
                                    for x in r_list[::-1]:
                                        accept_cmsn.prog_kill.pop(x)     #removing IDs, which have been killed already
                                    ...
                                    del back_kill_counter, r_list, z
            ...
            if a == 4:
                try:
                    with open(savename, 'r') as save:
                        ...
                        if len(kill_counter[0]) != len(kill_counter[1]):
                    ...
                    del line, temp_team, temp_weap, temp_item, temp_armo, temp_exp, temp_kill_cnt, exp_time, x, y, count
