...
#currently used: a,c,b,d,k,l,m,o,p,w,v,x,y,z
...
def menu_print(menu, c=0, d=0, sec_menu=[]): #length could be 24, but inputs take one space
    ...
    width, length = get_terminal_size((80, 24))        #getting terminal size
    length -= 1 + len(sec_menu)                     #reducing length of menu(because of either input() or pause())
...
def stat(char, c=0):      #(character, c=0:basic stats c=1:extended stats)
    if c == 0:                   #displays statictics
        out = f'{char.stat_name}- health:{char.stat_hp}/{char.stat_maxhp} attack:{char.atkmin}-{char.atkmax} total defence:{char.stat_defence}'
        if char.mana_max != 0:
...
#         return (char.stat_name + '- health:' + char.stat_hp + '/' + char.stat_maxhp + ' attack:' + char.atkmin + '-' + char.atkmax + ' total defence:' + char.stat_defence)
        ...
        out = f'{char.stat_name}- health:{char.stat_hp}/{char.stat_maxhp} attack:{char.atkmin}-{char.atkmax} total defence:{char.stat_defence} armor:{char.armor.stat_name}'
        ...
        if char.armor.ID != basal.ID:
            out += f' armor defence:{char.armor.stat_defence} weapon:{char.weapon.stat_name}'
        if char.mana_max != 0:
            out += f' mana:{char.stat_mana}/{char.stat_mana_max}'
        return out
#         x = ''
                ...
                if x.team == 'allies':
                    ...
                    if a == 1:
                        ...
                        while True:
                            ...
                            if x.mana_max != 0:                      #if character has mana pool
                                temp_menu.append(f'{y}. Magic')
                                contr.append(1)
                                y += 1
                            if known_skills:                        #if team(/c/h/a/r/a/c/t/e/r/) knows any skills
                            ...
                            if l == 1 and 1 in contr:               #if magic was chosen print available magic
                                for y, w in enumerate(known_magic):
                                    if y == 0:
                                        temp_menu = ['Magic:']
                                ...
                                for y, w in enumerate(known_skills):       #if skills was chosen print available skills
                                    v = ''
                                    if y == 0:
                                        temp_menu = ['Skills:']
                                    if w.cooldown != 0:
                            ...
                            v, k = in_err_ch(v, len(temp_menu)-1)
                        ...
                        if l == 1 and 1 in contr:        #for magic
                            x.t_comm, l = known_magic[v].use()
                        elif (l == 2 and len(contr) == 2) or (l == 1 and contr == [2]):   #for skills
                            x.t_comm, l = known_skills[v].use()
...
#             x.stat_set()
        ...
        for x in known_skills:          #reducing cooldown for skills by one
            x.cd_check()
...
def spf_doubl_atk(z, target):
    ...
    return f'{x.name} used double attack and dealt {dmg} damage to {target[o].name}'         #see item>use_item
...
sp_doubl_atk = sp_skill('Double attack', 0, spf_doubl_atk, 'atk', 30, 't2', 4, 1)

known_passive = []
known_skills = [sp_doubl_atk]
...
class Char:
    ID = 400
    def __init__ (self, name, maxhp, atk, atk_var, recr_cost=0, g_drop=0, exp_drop=0, mana_max = 0, defence=0, defence_type_1='none', weapon=basic, armor=basal, AI=AI_random):
...
#         print('self exp ' + str(self.exp) + ' , exp needed ' + str(exp_needed))
    ...
    def stat_set(self):
        ...
        self.stat_mana = stat_len(self.mana, 3)
...
Champion = Char('Champion', 90, 9, 2, 100)            #  magic_max=0 defence=0 defence_type='none' weapon=basic armor='basic')
...
#----imps----
imp_1 = Char('Lesser Imp',  40, 4, 2, 0, 20, 10, 0, 0, 'Subdemon', imp_bas_wep, imp_bas_arm)
imp_2 = Char('Imp Servant', 40, 4, 2, 0, 30, 20, 0, 0, 'Subdemon', imp_lig_wep, imp_lig_arm, AI=AI_servant)
imp_3 = Char('Imp',         60, 7, 1, 0, 50, 35, 0, 0, 'Subdemon', imp_lig_wep, imp_lig_arm, AI=AI_weakfirst)
imp_4 = Char('Imp Soldier', 60, 7, 1, 0, 60, 45, 0, 0, 'Subdemon', imp_med_wep, imp_med_arm, AI=AI_servant)
imp_5 = Char('Higher Imp',  120, 10, 0, 0, 100, 65, 0, 0, 'Demon', imp_lig_wep, imp_med_arm, AI=AI_onetarg)
imp_6 = Char('Imp General', 120, 10, 0, 0, 150, 80, 0, 0, 'Demon', imp_med_wep, imp_goo_arm, AI=AI_command)
imp_7 = Char('Imp Outcast', 120, 10, 0, 0, 220, 120, 0, 0, 'Demon', imp_goo_wep, imp_vgo_arm, AI=AI_strongfirst)

#----demons----
demon_1 = Char('demon_1', 100, 7, 2, 0, 180, 100, 0, 0, 'Demon', dem_bas_wep, dem_bas_arm)
demon_2 = Char('demon_2', 100, 7, 2, 0, 0, 0, 0, 0, 'Demon', dem_lig_wep, dem_lig_arm)
demon_3 = Char('demon_3', 150, 11, 1, 0, 0, 0, 0, 1, 'Demon', dem_lig_wep, dem_lig_arm)
demon_4 = Char('demon_4', 150, 11, 1, 0, 0, 0, 0, 1, 'Demon', dem_med_wep, dem_med_arm)
demon_5 = Char('demon_5', 270, 16, 2, 0, 0, 0, 0, 1, 'High Demon', dem_lig_wep, dem_med_arm)
demon_6 = Char('demon_6', 270, 16, 2, 0, 0, 0, 0, 1, 'High Demon', dem_med_wep, dem_goo_arm)
demon_7 = Char('demon_7', 270, 16, 2, 0, 0, 0, 0, 1, 'High Demon', dem_goo_wep, dem_vgo_arm)
