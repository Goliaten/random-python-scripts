...
def lis2cnt(lis):
    ...
    counter = [[], []]
    for x in lis:
        if not x.ID in [y.ID for y in counter[0]]:
            counter[0].append(x)
            counter[1].append(0)

        ind = [y.ID for y in counter[0]].index(x.ID)
        counter[1][ind] += 1
...
def nice_drop_comm(base):         #counter function, makes drop info a bit nicer
...
held_armor = [[light, heavy], [1, 2]]

if dev_opt == 'xel':
    held_weapons = list_2d_add(held_weapons, [[creator_touch], [3]])
    held_armor = list_2d_add(held_armor, [[creator_skin], [3]])
...
wood_stack = Mater('Wooden stack', 0, 'wood', 2)
...
demon_9 = Char('Demon Chosen',   350, 17, 3, 0, 300, 400, dem_def_drop_hig, dem_prb_drop_vhi, 0, 0, 2, 'High Demon', dem_goo_wep, dem_vgo_arm, AI=AI_onetarg)
...
cmsn_story_1 = Cmsn('First task',  1, 'hunt', [imp_2, imp_3], [2, 1], [], [], 100, [[potion, potionv2], [1, 1]])
cmsn_story_2 = Cmsn('Second task', 6, 'assasination', [imp_7, imp_6], [1, 1], [], [], 200, [[imp_soul], [1]])
cmsn_story_3 = Cmsn('Third task',  2, 'delivery', [], [], [met_nugget, met_ingot], [3, 1], 80, [[met_ingot], [2]])
cmsn_story_4 = Cmsn('Fourth task', 3, 'search')
cmsn_story_5 = Cmsn('Fifth task',  4, 'raid', [imp_3, imp_4], [5, 3], [met_ingot], [2], 250, [[bardiche], [1]])
...
def cmsn_gen(cmsn):
    ...
    type_num = randint(0, 3) #0=deliver, 1,2=hunt/assasination, 3=raid
#     type_num = 3
    ...
    if type_num in [1, 2, 3]:              #hunt/assasination
        ...
        rew_kill = rew_kill[randint(0, len(rew_kill)-1)]
        ...
        if type_num != 3:               #if it will be not raid
            ...
            _type = 'hunt' if [x for x in cmpl_kill_cnt if x != 1] else 'assasination'
                        
            names = hunt_name if _type == 'hunt' else assas_name         #get name templates from either hunt of assasination name list
            name_1 = names[randint(0, len(hunt_name if _type == 'hunt' else assas_name)-1)]   #choose one of the templates at random
    ...
    if type_num in [3]:                   #raid
        ...
        rew_gold = 0
        for y, x in enumerate(cmpl_kill):
            for _ in range(cmpl_kill_cnt[y]):
                rew_gold += x.g_drop
        for y, x in enumerate(cmpl_item):
            for _ in range(cmpl_item_cnt[y]):
                rew_gold += x.cost
        rew_gold = int(rew_gold * 0.4)
...
assas_name = ['Slay the *s', 'Hunt the *s', 'Eliminate the *s', '* elimination', 'Get rid of the *s', 'Problematic *']
...
#         cmsn_slot_1.print_all()
#         pause()
#         print(cmsn_slot_1.name)
#         y += 1
#         if y%10 == 0:
#             pause()
...
def cmsn_info(x, opt=0):
    ...
    if x.type in ['hunt', 'assasination', 'delivery', 'raid']:
...
#             out += ' '
    ...
    if x.rew_items[0]:
        out = ''
        for l, z in enumerate(x.rew_items[0]):
            cnt = x.rew_items[1][l]
            cnt = f'{cnt}*' if cnt > 1 else ''
            if l == 0:
                out += ' items: ' if len(x.rew_items[0]) > 1 else ' item: '
            out += f'{cnt}{z.name} '
            if l+1 < len(x.rew_items[0]):
                out += ', '
        ret.append(out)
                    ...
                    #---------complete (remind of) a comission------------
                        ...
                        while True:
                            ...
                            if a == 1:
                                ...
                                if accept_cmsn.type in ['delivery', 'raid']:
                                    
                                         #make copy of held_mater(?)(used to convert it into counter list)
                                    mater_counter = deepcopy(held_mater)
#                                     for x in held_mater:
...
#                                         del ind
                                    ...
                                    if deliv_items[0]:
                                        ...
                                        if o == 'yes':
                                            ...
                                            held_mater = deepcopy(mater_counter)
...
#                                                     held_mater.append(x)
                                ...
                                if not accept_cmsn.prog_kill + accept_cmsn.prog_item:
                                    ...
                                    for cnt, rew in enumerate(accept_cmsn.rew_items[0]):
                                        for _ in accept_cmsn.rew_items[1]:
                                            if type(rew) == Item:
                                                held_items = list_2d_add(held_items, [[rew], [1]])
                                            elif type(rew) == Mater:
                                                held_mater = list_2d_add(held_items, [[rew], [1]])
                                            elif type(rew) == Char:
                                                team_1.append(rew)
                                            elif type(rew) == Armor:
                                                held_armor = list_2d_add(held_items, [[rew], [1]])
                                            elif type(rew) == Weapon:
                                                held_weapons = held_2d_add(held_items, [[rew], [1]])
                                    ...
                                    #can i make it(V V V) into loop? idk, i guess i should 
                                    held_items = list_2d_sort(held_items)
        ...
        while True:
            ...
            if a == 3:
                with open(savename, 'w') as save:
                    ...
                    if held_items[0]:
                        save.write('itid')              #item id
                        for x in held_items[0]:
                            save.write(stat_len(x.ID, 4, 1, '0'))
                        save.write('---\n')
                        save.write('itcn')      #item counter
                        for x in held_items[1]:
                            save.write(stat_len(x, 4, 1, '0'))
                        save.write('---\n')
                        
                    if held_weapons[0]:
                        save.write('wpid')          #weapon id
                        for x in held_weapons[0]:
                            save.write(stat_len(x.ID, 4, 1, '0'))
                        save.write('---\n')
                        save.write('wpcn')      #weapon counter
                        for x in held_weapons[1]:
                            save.write(stat_len(x, 4, 1, '0'))
                        save.write('---\n')
                    
                    if held_armor[0]:
                        save.write('arid')         #armor id
                        for x in held_armor[0]:
                            save.write(stat_len(x.ID, 4, 1, '0'))
                        save.write('---\n')
                        save.write('arcn')      #armor counter
                        for x in held_armor[1]:
                            save.write(stat_len(x, 4, 1, '0'))
                        save.write('---\n')
                    
                    if held_mater[0]:
                        save.write('mtid')        #material id
                        for x in held_mater[0]:
                            save.write(stat_len(x.ID, 4, 1, '0'))
                        save.write('---\n')
                        save.write('mtcn')      #material counter
                        for x in held_mater[1]:
                            save.write(stat_len(x, 4, 1, '0'))
                        save.write('---\n')
                    
                    if kill_counter[0]:
                        save.write('kiid')           #kill counter id
                        for x in kill_counter[0]:
                            save.write(stat_len(x, 4, 1, '0'))
                        save.write('---\n')
                        save.write('kicn')          #kill counter counter (don't ask)
                    ...
                    if cmsn_kill_counter[0]:
                        ...
                        save.write('ckcn')         #comission kill counter
...
# after one package of data has been read, '---\n' is used to mark end of data pack
            ...
            if a == 4:
                try:
                    with open(savename, 'r') as save:
                        ...
                        while line != '':
                            ...
                            if line == 'char':
                                ...
                                while line != 'c--\n':
                                    line = save.read(4)
                                    if line == 'cpkd':       #comission progress kill id
                                        z = 'cpkd'
                                    elif line == 'cpkc':    #comission progress kill count
                                        z = 'cpkc'
                                    elif line == 'cpid':     #comission progress item id
                                        z = 'cpid'
                                    elif line == 'cpic':     #comission progress item count
                                        z = 'cpic'
                    ...
                    del line, temp_team, temp_weap, temp_item, temp_armo, temp_exp, temp_kill_cnt, temp_cmsn_kill_cnt, exp_time, x, count
