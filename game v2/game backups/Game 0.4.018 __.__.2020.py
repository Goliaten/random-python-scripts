...
def effect_init(q1, q2, q3, q4, AI_target, eff, init, targ, comm_1, comm_2):            #works only for team_1
    ...
    active_effects[key].x = q1               #assigning new x
    active_effects[key].key = key           #giving effect it's place in active effects to manipulate itself
    active_effects[key].src = init          #giving effecy information, about what initiated it
    
    if AI_target:            #checking if target is passed down
        if x.team == 'team 1' and targ == 'ally':      #checking which team it is from
    ...
    active_effects[key].start(v, q2)     #giving target and duration to effect and starting it
    
    if type(q3) == str:    #if initiator name is passed on
        passive_comm.append(f'{q3} now affects {v.name}')
    
    if AI_target:                        #if it's used in sp_atk
        return f'{comm_2} by {q1} for {q2} turns'
    else:                           #if it's used in item
        return None, v            #see Item>use_item

def i_atk_up(q1, q2, q3, q4, AI_target=None):
    return effect_init(q1, q2, q3, q4, AI_target, eff_atk_up, i_atk_up, 'ally', 'increase attack', 'increased attack')

def i_atk_down(q1, q2, q3, q4, AI_target=None):
    return effect_init(q1, q2, q3, q4, AI_target, eff_atk_down, i_atk_down, 'enem', 'lower attack', 'lowered attack')

def i_def_up(q1, q2, q3, q4, AI_target=None):
    return effect_init(q1, q2, q3, q4, AI_target, eff_def_up, i_def_up, 'ally', 'increase defence', 'increased defence')

def i_def_down(q1, q2, q3, q4, AI_target=None):
    return effect_init(q1, q2, q3, q4, AI_target, eff_def_down, i_def_down, 'enem', 'decrease defence', 'decreased defence')
...
#---magic attacks---
#                   SP_magic(name, cost, sph, spi, fnc_name, value_1, value_2, value_3, value_4 m_cost, level, upgrade)
...
sp_m_bf_atk_v4 = SP_magic('Buff attack v4', 0, sph_ally_ST, i_atk_up, 'buff atk ST', 6, 3,  'Buff attack v4', 0, 200, 4, None)
sp_m_bf_atk_v3 = SP_magic('Buff attack v3', 0, sph_ally_ST, i_atk_up, 'buff atk ST', 4, 4,  'Buff attack v3', 0, 170, 3, sp_m_bf_atk_v4)
sp_m_bf_atk_v2 = SP_magic('Buff attack v2', 0, sph_ally_ST, i_atk_up, 'buff atk ST', 2, 7,  'Buff attack v2', 0, 130, 2, sp_m_bf_atk_v3)
sp_m_bf_atk_v1 = SP_magic('Buff attack v1', 0, sph_ally_ST, i_atk_up, 'buff atk ST', 1, 10, 'Buff attack v1', 0, 80,  1, sp_m_bf_atk_v2)

sp_m_bf_def_v3 = SP_magic('Buff defence v3', 0, sph_ally_ST, i_def_up, 'buff def ST', 3, 7, 'Buff defence v3', 0, 180, 3, None)
sp_m_bf_def_v2 = SP_magic('Buff defence v2', 0, sph_ally_ST, i_def_up, 'buff def ST', 2, 5, 'Buff defence v2', 0, 130, 2, sp_m_bf_def_v3)
sp_m_bf_def_v1 = SP_magic('Buff defence v1', 0, sph_ally_ST, i_def_up, 'buff def ST', 1, 4, 'Buff defence v1', 0, 80,  1, sp_m_bf_def_v2)

sp_m_dbf_atk_v3 = SP_magic('Debuff attack v3', 0, sph_enem_ST, i_atk_down, 'debuff atk ST', 6, 3, 'Debuff attack v3', 0, 170, 3, None)
sp_m_dbf_atk_v2 = SP_magic('Debuff attack v2', 0, sph_enem_ST, i_atk_down, 'debuff atk ST', 4, 4, 'Debuff attack v2', 0, 130, 2, sp_m_dbf_atk_v3)
sp_m_dbf_atk_v1 = SP_magic('Debuff attack v1', 0, sph_enem_ST, i_atk_down, 'debuff atk ST', 2, 7, 'Debuff attack v1', 0, 90,  1, sp_m_dbf_atk_v2)

sp_m_dbf_def_v3 = SP_magic('Debuff defence v3', 0, sph_enem_ST, i_def_down, 'debuff def ST', 3, 4, 'Debuff defence v3', 0, 170, 3, None)
sp_m_dbf_def_v2 = SP_magic('Debuff defence v2', 0, sph_enem_ST, i_def_down, 'debuff def ST', 2, 4, 'Debuff defence v2', 0, 140, 2, sp_m_dbf_def_v3)
sp_m_dbf_def_v1 = SP_magic('Debuff defence v1', 0, sph_enem_ST, i_def_down, 'debuff def ST', 1, 3, 'Debuff defence v1', 0, 90,  1, sp_m_dbf_def_v2)

#---skill attacks---
               #SP_skill(name, cost, sph, spi, fnc_name, value_1, value_2, value_3, value_4, cooldown, init_cooldown, level, upgrade):
...
#---passive attacks(if you can call them attacks)---
                  #SP_passive(name, cost, sph, spi, fnc_name, value_1, value_2, value_3, value_4, level, upgrade)
...
#----i may do 'class' sub-classes of Char just because some of character do not use some values or use mass[]
class Char:
...
class Item:
    ...
    def use_item (self):
        ...
        t_comm, o = self.fnc(self.value_1, self.value_2, self.value_3, self.value_4)    #using function assigned to itself
        
        try:                  #----proposed to rewrite, but may cause problems if item is chosen but not used----
