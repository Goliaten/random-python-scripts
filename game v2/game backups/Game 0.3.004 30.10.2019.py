...
def sp_healall(z, target):
    ...
    for y in target:
        ...
        if y.hp > y.maxhp:                     #prevent overheal
            y.hp = y.maxhp
        y.stat_set()
    return x.name + ' used mass healing magic'
...
sp_heal = sp_magic('Heal', 0, sp_healall, 'heal', 30, 't1', 20)
...
#they can be written in any order(except experience, which should be read after armor), since loading operates on headers to write data
