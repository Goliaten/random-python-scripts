...
def sph_ally_MT(AI_target):  #1 target is chosen manually, 2nd is chosen randomly
    ...
    while True:
        if len(team) == 1:            #if team consists of only 1 character, second target is not randomised
            break
        y = randint(0, len(team))       #randomising the second target
...
def spi_dmg(target, q1, q2):   #(target=target from sph, q1 and q2 =value from sp_attack class itself)  (q1 is damageto be dealt)
...
def spi_heal(target, q1, q2):        #q1 is amount of heal
...
def spi_atk_strong(target, q1, q2):   #stronger attack(q1 is multiplier)
    dmg = int(x.deal_dmg() * q1)
    target.take_dmg(dmg)
    return f'dealt {dmg} damage'

def spi_atk_multi(target, q1, q2):   #for attacking multiple times  (q1 is attack number)
...
def spi_atk_arm(target, q1, q2):   #attack penetrating armor(q1 is armor penetration)
    q1 = q1 if q1 < target.defence else target.defence
    dmg = x.deal_dmg() + q1
    target.take_dmg(dmg)
    return f'dealt {dmg} damage'

def spi_hp_to_mana(target, q1, q2):
    if target.hp <= q1:
        q1 = target.hp
        'fatal damage'
    target.take_dmg(q1)
    ...
    return f'restored {mana_g} by dealing {q1} damage'
...
class SP_passive(SP_atk):
    ...
    def activate(self, AI_target=None):
        ...
        action = self.spi(target, self.value_1, self.value_2)
...
test_boi_5 = Char('_test_boi_ 5', 100, 10, 0, mana_max=1000, mana_gain=100,
                  ...
                  skill_tree=[[1,1,1, 1]],
                  magic=[sp_m_heal_ST_v1, sp_m_heal_MT_v1, sp_m_heal_AT_v1, sp_m_dmg_ST_v1, sp_m_dmg_MT_v1, sp_m_dmg_AT_v1],
                  magic_tree=[[1,1,1,1,1,1]],
                  tree_step=[1])
