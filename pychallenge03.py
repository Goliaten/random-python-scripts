...
def fnc(inp):
    ...
    for y, x in enumerate(inp):
        ...
        rl_lis = [l2, l3, l4, r1, r2, r3]
        s_lis = [l1, s, r4]
        for z in rl_lis:
            if not ord(z) in range(65, 91):
                flag = 1
                break
        for z in s_lis:
            if not ord(z) in range(97, 123):
                flag = 1
                break
            
            
        if flag == 0:
            out.append(f'{l1+l2+l3+l4+s+r1+r2+r3+r4}')
        
        
#        if [z for z in rl_lis if ord(z) in range(65, 91)] + [l for x in [s]if ord(l) in range(97, 123)]:
#            out.append(f'{l1+l2+l3+s+r1+r2+r3}')
