            ...
            while z == 0:
                ...
                if x.team == 'allies':
                    ...
                    if a == 1:
                        ...
                        if k == None:      #checking if item was used
                            x.t_comm = t_target
...
#item function         #remember to add at every item t_target(turn target/target for effect) for EoT_summ
        #change k to None if item is not used and change t_target
...
def revival(hp, c=0):  #hp for health after revivalion, c=0 for ally ress c=1 for enem ress
    ...
    while True:
        ...
        if not z:         #-------------this may be used often in future-----------------------------
            ...
            t_target = x.name + ' tries to find anyone that may be in dead need, but he just loses time(and turn)'
            break
...
class Char:
    ID = 400
    def __init__ (self, name, maxhp, atk, atk_var, recr_cost=0, g_drop=0, exp_drop=0, defence=0, defence_type_1='none', weapon=basic, armor=basal):
...
class Item:
    ...
    def use_item (self):
        ...
        if k != None:                   #if item was actually used
...
imp_2 = Char('Imp Servant', 40, 4, 2, 0, 30, 20, 0, 'Subdemon', imp_lig_wep, imp_lig_arm)
imp_3 = Char('Imp',         60, 7, 1, 0, 50, 35, 0, 'Subdemon', imp_lig_wep, imp_lig_arm)
imp_4 = Char('Imp Soldier', 60, 7, 1, 0, 60, 45, 0, 'Subdemon', imp_med_wep, imp_med_arm)
imp_5 = Char('Higher Imp',  120, 10, 0, 0, 100, 65, 0, 'Demon', imp_lig_wep, imp_med_arm)
imp_6 = Char('Imp General', 120, 10, 0, 0, 150, 80, 0, 'Demon', imp_med_wep, imp_goo_arm)
imp_7 = Char('Imp Outcast', 120, 10, 0, 0, 220, 120, 0, 'Demon', imp_goo_wep, imp_vgo_arm)
