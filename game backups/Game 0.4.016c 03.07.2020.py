...
def sph_enem_AT(AI_target):        #for comments see sph_ally
    if x.team == 'team 1':
        return team_2, 'team 2'
    elif x.team == 'team 2':
        return team_1, 'team 1'
        ...
        raise(Exception(f'Wrong team for {x.name}: {x.team}'))
def sph_ally_ST(AI_target):        #for comments see sph_ally
    global p
...
def sph_enem_ST(AI_target):        #for comments see sph_ally
    global p
    if AI_target == None:          #if player has option to choose target
    ...
    p = target
    return target, target.name
def sph_self(AI_target):        #for comments see sph_ally
...
sp_m_dmg_ST_v3 = SP_magic('Damage one v3', 0, sph_enem_ST, spi_dmg, 'dmg ST', 20, 0, 90, 3, None)
sp_m_dmg_ST_v2 = SP_magic('Damage one v2', 0, sph_enem_ST, spi_dmg, 'dmg ST', 14, 0, 75, 2, sp_m_dmg_ST_v3)
sp_m_dmg_ST_v1 = SP_magic('Damage one v1', 0, sph_enem_ST, spi_dmg, 'dmg ST', 10, 0, 50, 1, sp_m_dmg_ST_v2)
...
#skipping MT magic, as there is none now
...
def AI_mage_heal(char):
    ...
    for y, ally in enumerate(team):
        ...
        for magic in ST_heals:
            if magic.value_1 <= m_h + loss_toler:    #if value healed is less or equal to missing hp + loss tolerancy
                psbl_heals[0].append(magic)
                psbl_heals[1].append(y)
    
    for magic in AT_heals:
        if magic.value_1 <= (team_m_h + loss_toler) / len(team):
            psbl_heals[0].append(magic)
    ...
    #--------using attack is can't use magic--------
    if opt_atk == 1:
...
test_boi_5 = Char('_test_boi_ 5', 100, 10, 0, mana_max=1000, mana_gain=100,
                  ...
                  skill_tree=[[1,1,1, 1]],
                  magic=[sp_m_heal_ST_v1, sp_m_heal_AT_v1, sp_m_dmg_ST_v1, sp_m_dmg_AT_v1],
                  magic_tree=[[1,1,1,1]],
                  tree_step=[1])
...
test_boi_6 = Char('_test_boi_ 6', 500, 1, 0, mana_max=200, mana_gain=10, AI=AI_mage_heal,    #hit me senpai
                  magic=[sp_m_heal_ST_v1, sp_m_heal_AT_v1],
                  magic_tree=[[1,1]],
                  tree_step=[1])
