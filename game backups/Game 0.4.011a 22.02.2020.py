...
def fight_end(c, g_gain=0, exp_gain=0, def_drop=[], prb_drop=[]):
    ...
    global gold, held_mater, temp_menu
    for x in team_1:
        x.heal_up()
    if c == 1:         #if team_1 won
        
        temp_menu = ['You won the fight']                        #(prepare to)print some info
#         g_gain = g_gain if g_gain else 0                    #gold gained
#         exp_gain = exp_gain if exp_gain else 0                     #exp gained
...
def revival(hp, c=0):  #hp for health after revivalion, c=0 for ally ress c=1 for enem ress
    ...
    while True:
        ...
        o, z = in_err_ch(o, len(sec_menu)-1)
...
held_mater = [imp_horn, imp_heart, imp_soul, met_nugget, met_ingot, wood_plank, met_nugget, met_nugget, met_nugget]   #held/owned materials
...
cmsn_story_1 = Cmsn('First task',  1, 'hunt', ['Imp Servant', 'Imp'], [2, 1], [], [], 100, ['Potion', 'Potion v2'])
cmsn_story_2 = Cmsn('Second task', 6, 'assasination', ['Imp Outcast', 'Imp General'], [1, 1], [], [], 200, ['Imp soul'])
...
cmsn_story_5 = Cmsn('Fifth task',  4, 'raid', ['Imp', 'Imp Soldier'], [5, 3], ['Iron Ingot'], [2], 250, ['Bardiche'])
...
# Ah yes, the only value, that does not need any class is gold
gold = 100
...
demon_7 = Char('Demon Warlord',  270, 16, 2, 0, 400, 300, [], [], 0, 0, 1, 'High Demon',    dem_goo_wep, dem_vgo_arm, AI=AI_command)
demon_8 = Char('Demon Chosen',   350, 17, 3, 0, 600, 400, [], [], 0, 0, 2, 'High Demon',    dem_goo_wep, dem_vgo_arm, AI=AI_onetarg)
                    ...
                    if a == 1:
                        while True:
                            ...
                            if a == 1:
                                ...
                                o = input('Which one? ')
                                o, z = in_err_ch(o, len(temp_cmsn), 0)
                                ...
                                while True:
                                    ...
                                    if accept_cmsn:
                                        ...
                                        o = input('Type "yes" to accept a new comission and forget other one.')
                                        if o != 'yes':
                                            break
                        ...
                        while True:
                            ...
                            sec_menu = ['1. Complete this comission',
                            ...
                            if a == 1:
                                ...
                                temp_menu = [
                                    'Comission:',
                                    f'Name: {accept_cmsn.name}',
                                    ]
                                
                                sec_menu = []
                                    
                                if accept_cmsn.type == 'hunt':
                                    pass
                                
                                elif accept_cmsn.type == 'assasination':
                                    pass
                                    ...
                                    #VVV checking how many materials held match those needed
                                    temp_cnt = []
                                    
                                    for temp_item, temp_item_cnt in zip(accept_cmsn.prog_item, accept_cmsn.prog_item_cnt): #going over progress left to complete a quest
                                        cnt = 0                             #amount of currently iterated item, that is held/owned
                                        
                                        for item in held_mater:
                                            if temp_item.ID == item.ID:      #if iterated Materials is held/owned
                                                cnt += 1                     #increase the counter
                                            if temp_item_cnt == cnt:         #if counter is equal to completion progress left
                                                break                        #break(and begin iteration with next required item)
                                        
                                        temp_cnt.append(cnt)               #oh, and append counter to list that collects it for reason below
                                        
                                    
                                    #--------------------------------------
                                    #VVV checking if any of these items can be delivered all at once (DAAO)
                                    break_num = 0                                           #used to break loop below
                                    z = 0                                                   #control variable
                                    compl_item = []                                         #indexes of items, that can be DAAO
                                    
                                    for y, x in enumerate(temp_cnt):
                                        if temp_cnt[y] == accept_cmsn.prog_item_cnt[y]:
                                            compl_item.append(y)
                                            
                                        else:
                                            break_num += 1
                                            if break_num == len(temp_cnt):                  #if number of times, that there were not enough material to DAAO
                                                z = 1
                                                break                                       #set a flag(z) and break
                                    
                                    #--------------------------------------
                                    if z == 0:
                                        temp_menu.append('Items, that can be DAAO:')
                                        for y in compl_item:
                                            sec_menu.append(f' {accept_cmsn.prog_item[y].name}x{accept_cmsn.prog_item_cnt[y]}')
                                            
                                        o = input('Do you want to deliver them?(type "ex" to exit) ')
                                        
                                        print([x.name for x in held_mater])
                                        
                                        
                                        if o != "ex":
                                            print(f'compl_item: {compl_item}')
                                            
                                            for y in compl_item[::-1]:         #getting indexes of items DAAO in reverse, so progress items can be removed from accepted comission here
                                                print(f'y: {y}')
                                                temp_num = 0                #count how many items to remove(remove count number)
                                                
                                                print(f'held_mater: {[x.name for x in held_mater]}')
                                                
                                                    
                                                for item in held_mater:                #searching for material in list
                                                    print(f'  item: {item.name}, accept_cmsn.prog_item[{y}]: {accept_cmsn.prog_item[y].name}')
                                                    
                                                    if item.ID == accept_cmsn.prog_item[y].ID:     #if one is found, increase remove count number
                                                        temp_num += 1
                                                    if temp_num == temp_cnt[y]:                #if enough items were found, go to next step
                                                        break
                                                
                                                print(f'temp_ind: {temp_ind}')
                                                
                                                print(f'len temp_ind: {len(temp_ind)}; len temp_cnt[{y}]: {temp_cnt[y]}')
                                                if temp_num != temp_cnt[y]:               #if amount of items found here is different than one before
                                                    raise Exception('GatherCmsnPopError')
                                                
                                                print(f'temp_ind[::-1]: {temp_ind[::-1]}')
                                                for z in range(temp_num):           #removing f'{temp_num}' items from the held materials
                                                    #     V remove       V on index where V this item is at
                                                    held_mater.pop(held_mater.index(accept_cmsn.prog_item[y]))
                                                    
                                                
                                                accept_cmsn.prog_item.pop(y)        #and remove progress requirements from comission
                                                accept_cmsn.prog_item_cnt.pop(y)
                                        ...
                                        print('Not enough items to deliver')
                                        print()
                                    
                                    
                                    del temp_cnt, break_num, z, compl_item, temp_ind
            ...
            if a == 1:
                ...
                while True:
                    ...
                    if a == 2 and team_1[l].weapon.ID != basic.ID:                         #if char already has a weapon
                        ...
                        x = input(f'Do you want to unequip {team_1[l].weapon.name} and equip {held_weapons[o].name}? (yes to confirm) ')
                        ...
                        x = input(f'Do you want to unequip {team_1[l].armor.name} armor and equip {held_armor[o].name} armor? (yes to confirm) ')
                        if x != 'yes':
                            continue
...
#after one package of data has been read, '---\n' is used to mark end of data pack
            ...
            if a == 4:
                try:
                    ...
                    menu_print([f'File {savename} is outdated'], 1)
                    pause()
