...
#passive, skill, magic
class sp_atk:
    def __init__(self, name, cost, sph, spi, fnc_name, value):
        global ID
        self.ID = ID
        ID += 1
        self.name = name
...
class sp_passive(sp_atk):
    def __init__(self, name, cost, sph, spi, fnc_name, value):
        super().__init__(name, cost, sph, spi, fnc_name, value)
    def activate(self):
...
class sp_skill(sp_atk):
    def __init__(self, name, cost, sph, spi, fnc_name, value, cooldown, init_cooldown):
        super().__init__(name, cost, sph, spi, fnc_name, value)
        self.cooldown = 1
...
class sp_magic(sp_atk):
    def __init__(self, name, cost, sph, spi, fnc_name, value, m_cost):    #m_cost = mana cost
        super().__init__(name, cost, sph, spi, fnc_name, value)
        self.m_cost = m_cost
    def use(self):
#         print(x.mana)
...
sp_heal_all_v1 = sp_magic('Heal all v1', 0, sph_t1, spi_heal, 'heal', 20, 20)
sp_heal_all_v2 = sp_magic('Heal all v2', 0, sph_t1, spi_heal, 'heal', 30, 40)
sp_dmg_all = sp_magic('Damage all', 0, sph_t2, spi_dmg, 'dmg', 10, 50)

sp_doubl_atk = sp_skill('Double attack', 0, sph_t2_targ, spi_atk_multi, 'atk', 2, 4, 1)
sp_str_atk = sp_skill('Strong attack', 0, sph_t2_targ, spi_atk_strong, 'atk', 1.5, 4, 2)

sp_pass_heal_all = sp_passive('Passive heal allies', 0, sph_t1, spi_heal, 'heal', 2)
sp_pass_dmg_all = sp_passive('Passive damage enemies', 0, sph_t2, spi_dmg, 'dmg', 2)


# sp_heal_all_v1 = sp_magic('Heal all 1', 0, spf_heal_all, 'heal', 20, 'team 1', 20)
# sp_heal_all_v2 = sp_magic('Heal all 2', 0, spf_heal_all, 'heal', 30, 'team 2', 40)
# sp_dmg_all = sp_magic('Damage all', 0, spf_dmg_all, 'dmg', 10, 'team 2', 50)
# 
# sp_doubl_atk = sp_skill('Double attack', 0, spf_doubl_atk, 'atk', 0, 'team 2', 4, 1)
# sp_str_atk = sp_skill('Strong attack', 0, spf_str_atk, 'atk', 1.5, 'team 2', 4, 2)
# 
# sp_pass_heal_all = sp_passive('Passive heal', 0, spf_pass_heal, 'heal', 2, 'team 1')
# sp_pass_dmg_all = sp_passive('Passive damage', 0, spf_pass_dmg, 'dmg', 2, 'team 2')
...
# known_passive, known_magic, known_skills = [], [], []
...
dev_global_weapons = [imp_bas_wep, imp_bas_wep, imp_med_wep, imp_goo_wep, dem_bas_wep, dem_lig_wep, dem_med_wep, dem_goo_wep,
                 bat, sword, hammer, heal_sword, creator_touch]
all_id_class += dev_global_weapons   #for checking all ID

#-----Armor------
#----user-armor----
basal = Armor('None', 0, 0)  #name cost defence defence_type='none'
light = Armor('Light armor', 100, 2)
medial = Armor('Medium armor', 200, 6)
heavy = Armor('Heavy armor', 350, 10)
...
class Char:
    def __init__ (self, name, hp_max, atk, atk_var, recr_cost=0, g_drop=0, exp_drop=0, mana_max=0, mana_gain=5, defence=0,
                  defence_type_1='none', weapon=basic, armor=basal, AI=AI_random, skills=[], magic=[], passive=[]):
        global ID
...
#         print('self exp ' + str(self.exp) + ' , exp needed ' + str(exp_needed))
        ...
        while self.exp >= exp_needed:               #if required experience is gained and while it is bigger than requirments
            ...
            lvl_change_dict = {
                'hp': [0, self.base_hp_max, self.hp_max, 20, 10],     #zero serves as index, first in list is stat without equipment/buff/skill
...
#  mana_max=0 mana_gain=0 defence=0 defence_type='none' weapon=basic armor='basic', AI=AI_random, skills=[], magic=[], passive=[])

# known_passive = [sp_pass_heal_all, sp_pass_dmg_all]
# known_skills = [sp_doubl_atk, sp_str_atk]
# known_magic = [sp_heal_all_v1, sp_heal_all_v2, sp_dmg_all]

hero = Char('Hero', 70, 10, 1, 100, defence=1, skills=[sp_doubl_atk, sp_str_atk])
champion = Char('Champion', 90, 9, 2, 100, passive=[sp_pass_heal_all, sp_pass_dmg_all])
...
#(name hp_max atk atk_var recruitment_cost=0 g_drop=0 exp_drop=0
#  magic_max=0 mana_gain=0 defence=0 defence_type='none' weapon=basic armor='basic', AI=AI_random, skills=[], magic=[], passive=[])
...
imp_7 = Char('Imp Outcast', 120, 10, 0, 0, 185, 100, 0, 0, 0, 'Demon', imp_goo_wep, imp_vgo_arm, AI=AI_strongfirst)

#----demons----
demon_1 = Char('Demon Slave',    100, 7,  2, 0, 70,  50,  0, 0, 0, 'Demon', dem_bas_wep, dem_bas_arm, AI=AI_servant)
demon_2 = Char('Lower Demon',    100, 7,  2, 0, 80,  58,  0, 0, 0, 'Demon', dem_lig_wep, dem_lig_arm, AI=AI_weakfirst)
demon_3 = Char('Demon',          150, 11, 1, 0, 120, 70,  0, 0, 1, 'Demon', dem_lig_wep, dem_lig_arm)
demon_4 = Char('Demon Fighter',  150, 11, 1, 0, 155, 85,  0, 0, 1, 'Demon', dem_med_wep, dem_med_arm, AI=AI_onetarg)
demon_5 = Char('Higher Demon',   270, 16, 2, 0, 220, 120, 0, 0, 1, 'High Demon', dem_lig_wep, dem_med_arm, AI=AI_strongfirst)
demon_6 = Char('Demon Guardian', 270, 16, 2, 0, 250, 135, 0, 0, 1, 'High Demon', dem_med_wep, dem_goo_arm, AI=AI_servant)
demon_7 = Char('Demon Lord',     270, 16, 2, 0, 400, 200, 0, 0, 1, 'High Demon', dem_goo_wep, dem_vgo_arm, AI=AI_command)
                    ...
                    if a == 1:
                        ...
                        while True:
                            for x in team_1:
                                ...
                                if not x.magic + x.skills + x.passive:
