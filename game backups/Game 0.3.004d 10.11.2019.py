...
#         return (char.stat_name + '- health:' + char.stat_hp + '/' + char.stat_maxhp + ' attack:' + char.atkmin + '-' + char.atkmax + ' total defence:' + char.stat_defence)
        ...
        out = f'{stat_len(char.name, leng+1)}- health:{char.stat_hp}/{char.stat_maxhp} attack:{char.atkmin}-{char.atkmax} total defence:{char.stat_defence} armor:{char.armor.stat_name}'
...
#         print('self exp ' + str(self.exp) + ' , exp needed ' + str(exp_needed))
        ...
        if self.exp >= exp_needed:       #if required experience is gained
            ...
            self.heal_up()
    def stat_set(self):
        self.stat_name = stat_len(self.name, 14)            #for stat purpose
...
demon_7 = Char('Demon Lord', 270, 16, 2, 0, 0, 0, 0, 0, 1, 'High Demon', dem_goo_wep, dem_vgo_arm)
...
enem_19 = [demon_7, demon_6, demon_6, demon_6, demon_6]
enem_20 = []
