...
#------------------item-fnc----------------------------
def i_atk_up(z, y, AI_target=None):            #works only for team_1
    global buff_count, o
    ...
    if AI_target:            #checking if target is passed down
        if x.team == 'team 1':      #checking which team it is from
    ...
    if AI_target:                        #if it's used in sp_atk
        return f'increased attack by {z} for {y} turns'
    else:                           #if it's used in item
        return None, v            #see Item>use_item

def i_atk_down(z, y, AI_target=None):            #works only for team_1
    global buff_count, o
    buff_count += 1        #for creating keys
    key = str(buff_count)  #creating new key
    ...
    if AI_target:            #checking if AI script is controlling the character
        if x.team == 'team 1':      #checking which team it is from
            team = team_2               #assigning correct team to variable
        elif x.team == 'team 2':
            team = team_1
        o = team.index(AI_target)              #this function uses o in stead of AI_target
    else:
    ...
    if AI_target:                        #if it's used in sp_atk
        return f'lowered attack by {z} for {y} turns'
    else:                           #if it's used in item
        return None, v            #see Item>use_item

def i_def_up(z, y, AI_target=None):
    global buff_count, o
    buff_count += 1
    key = str(buff_count)
    
    active_effects[key] = deepcopy(eff_def_up)
    active_effects[key].x = z
    active_effects[key].key = key
    ...
    if AI_target:            #checking if AI script is controlling the character
        ...
        team_target(1, [f'Who does {x.name} want to increase defence of?'])
        team = team_1
    v = team[o]         #choosing target for effect
        
    active_effects[key].start(v, y)
    
    if AI_target:                        #if it's used in sp_atk
        return f'increased defence by {z} for {y} turns'
    else:                           #if it's used in item
        return None, v            #see Item>use_item

def i_def_down(z, y, AI_target=None):
    global buff_count, o
    buff_count += 1
    key = str(buff_count)
    
    active_effects[key] = deepcopy(eff_def_down)
    active_effects[key].x = z
    active_effects[key].key = key
    ...
    if AI_target:            #checking if AI script is controlling the character
        ...
        team_target(2, [f'Who does {x.name} want to decrease defence of?'])
        team = team_2
    v = team[o]         #choosing target for effect
    ...
    if AI_target:                        #if it's used in sp_atk
        return f'decreased defence by {z} for {y} turns'
    else:                           #if it's used in item
        return None, v            #see Item>use_item
...
def spi_dmg(q1, q2, target):   #(target=target from sph, q1 and q2 =value from sp_attack class itself)  (q1 is damageto be dealt)
...
def spi_heal(q1, q2, target):        #q1 is amount of heal
...
def spi_atk_strong(q1, q2, target):   #stronger attack(q1 is multiplier)
    dmg = int(x.deal_dmg() * q1)
    target.take_dmg(dmg)
    return f'dealt {dmg} damage'

def spi_atk_multi(q1, q2, target):   #for attacking multiple times  (q1 is attack number)
...
def spi_atk_arm(q1, q2, target):   #attack penetrating armor(q1 is armor penetration)
    q1 = q1 if q1 < target.defence else target.defence
    dmg = x.deal_dmg() + q1
    target.take_dmg(dmg)
    return f'dealt {dmg} damage'

def spi_hp_to_mana(q1, q2, target):
    if target.hp <= q1:
        q1 = target.hp
        'fatal damage'
    target.take_dmg(q1)
...
class SP_atk:
    ...
    def __init__(self, name, cost, sph, spi, fnc_name, value_1, value_2, level, upgrade):
...
class SP_passive(SP_atk):
    ...
    def __init__(self, name, cost, sph, spi, fnc_name, value_1, value_2, level, upgrade):
        ...
        super().__init__(name, cost, sph, spi, fnc_name, value_1, value_2, level, upgrade)
    ...
    def activate(self, AI_target=None):
        ...
        action = self.spi(self.value_1, self.value_2, target)
...
class SP_skill(SP_atk):
    ...
    def __init__(self, name, cost, sph, spi, fnc_name, value_1, value_2, cooldown, init_cooldown, level, upgrade):
...
class SP_magic(SP_atk):
    ...
    def __init__(self, name, cost, sph, spi, fnc_name, value_1, value_2, m_cost, level, upgrade):    #m_cost = mana cost
...
#---magic attacks---
#                   SP_magic(name, cost, sph, spi, fnc_name, value_1, value_2, m_cost, level, upgrade)
sp_m_heal_ST_v3 = SP_magic('Heal one v3', 0, sph_ally_ST, spi_heal, 'heal ST', 40, 0, 100, 1, None)
sp_m_heal_ST_v2 = SP_magic('Heal one v2', 0, sph_ally_ST, spi_heal, 'heal ST', 30, 0, 75,  1, sp_m_heal_ST_v3)
sp_m_heal_ST_v1 = SP_magic('Heal one v1', 0, sph_ally_ST, spi_heal, 'heal ST', 20, 0, 40,  1, sp_m_heal_ST_v2)

sp_m_heal_MT_v3 = SP_magic('Heal two v3', 0, sph_ally_MT, spi_heal, 'heal MT', 30, 20, 100, 3, None)
sp_m_heal_MT_v2 = SP_magic('Heal two v2', 0, sph_ally_MT, spi_heal, 'heal MT', 23, 15, 75,  2, sp_m_heal_MT_v3)
sp_m_heal_MT_v1 = SP_magic('Heal two v1', 0, sph_ally_MT, spi_heal, 'heal MT', 15, 10, 50,  1, sp_m_heal_MT_v2)

sp_m_heal_AT_v3 = SP_magic('Heal all v3', 0, sph_ally_AT, spi_heal, 'heal AT', 20, 0, 100, 3, None)
sp_m_heal_AT_v2 = SP_magic('Heal all v2', 0, sph_ally_AT, spi_heal, 'heal AT', 15, 0, 75,  2, sp_m_heal_AT_v3)
sp_m_heal_AT_v1 = SP_magic('Heal all v1', 0, sph_ally_AT, spi_heal, 'heal AT', 10, 0, 50,  1, sp_m_heal_AT_v2)

sp_m_dmg_ST_v3 = SP_magic('Damage one v3', 0, sph_enem_ST, spi_dmg, 'dmg ST', 22, 0, 90, 3, None)
sp_m_dmg_ST_v2 = SP_magic('Damage one v2', 0, sph_enem_ST, spi_dmg, 'dmg ST', 16, 0, 75, 2, sp_m_dmg_ST_v3)
sp_m_dmg_ST_v1 = SP_magic('Damage one v1', 0, sph_enem_ST, spi_dmg, 'dmg ST', 12, 0, 50, 1, sp_m_dmg_ST_v2)

sp_m_dmg_MT_v3 = SP_magic('Damage two v3', 0, sph_enem_MT, spi_dmg, 'dmg MT', 17, 14, 100, 3, None)
sp_m_dmg_MT_v2 = SP_magic('Damage two v2', 0, sph_enem_MT, spi_dmg, 'dmg MT', 13, 9,  75,  2, sp_m_dmg_MT_v3)
sp_m_dmg_MT_v1 = SP_magic('Damage two v1', 0, sph_enem_MT, spi_dmg, 'dmg MT', 9,  6,  50,  1, sp_m_dmg_MT_v2)

sp_m_dmg_AT_v3 = SP_magic('Damage all v3', 0, sph_enem_AT, spi_dmg, 'dmg AT', 10, 0, 100, 3, None)
sp_m_dmg_AT_v2 = SP_magic('Damage all v2', 0, sph_enem_AT, spi_dmg, 'dmg AT', 8,  0, 70,  2, sp_m_dmg_AT_v3)
sp_m_dmg_AT_v1 = SP_magic('Damage all v1', 0, sph_enem_AT, spi_dmg, 'dmg AT', 6,  0, 50,  1, sp_m_dmg_AT_v2)

sp_m_bf_atk_v4 = SP_magic('Buff attack v4', 0, sph_ally_ST, i_atk_up, 'buff atk ST', 6, 3,  200, 4, None)
sp_m_bf_atk_v3 = SP_magic('Buff attack v3', 0, sph_ally_ST, i_atk_up, 'buff atk ST', 4, 4,  170, 3, sp_m_bf_atk_v4)
sp_m_bf_atk_v2 = SP_magic('Buff attack v2', 0, sph_ally_ST, i_atk_up, 'buff atk ST', 2, 7,  130, 2, sp_m_bf_atk_v3)
sp_m_bf_atk_v1 = SP_magic('Buff attack v1', 0, sph_ally_ST, i_atk_up, 'buff atk ST', 1, 10, 80,  1, sp_m_bf_atk_v2)

sp_m_bf_def_v3 = SP_magic('Buff defence v3', 0, sph_ally_ST, i_def_up, 'buff def ST', 3, 7, 180, 3, None)
sp_m_bf_def_v2 = SP_magic('Buff defence v2', 0, sph_ally_ST, i_def_up, 'buff def ST', 2, 5, 130, 2, sp_m_bf_def_v3)
sp_m_bf_def_v1 = SP_magic('Buff defence v1', 0, sph_ally_ST, i_def_up, 'buff def ST', 1, 4, 80,  1, sp_m_bf_def_v2)

sp_m_dbf_atk_v3 = SP_magic('Debuff attack v3', 0, sph_enem_ST, i_atk_down, 'debuff atk ST', 6, 3, 170, 3, None)
sp_m_dbf_atk_v2 = SP_magic('Debuff attack v2', 0, sph_enem_ST, i_atk_down, 'debuff atk ST', 4, 4, 130, 2, sp_m_dbf_atk_v3)
sp_m_dbf_atk_v1 = SP_magic('Debuff attack v1', 0, sph_enem_ST, i_atk_down, 'debuff atk ST', 2, 7, 90,  1, sp_m_dbf_atk_v2)

sp_m_dbf_def_v3 = SP_magic('Debuff defence v3', 0, sph_enem_ST, i_def_down, 'debuff def ST', 3, 4, 170, 3, None)
sp_m_dbf_def_v2 = SP_magic('Debuff defence v2', 0, sph_enem_ST, i_def_down, 'debuff def ST', 2, 4, 140, 2, sp_m_dbf_def_v3)
sp_m_dbf_def_v1 = SP_magic('Debuff defence v1', 0, sph_enem_ST, i_def_down, 'debuff def ST', 1, 3, 90,  1, sp_m_dbf_def_v2)

#---skill attacks---
               #SP_skill(name, cost, sph, spi, fnc_name, value_1, value_2, cooldown, init_cooldown, level, upgrade):
sp_s_atk_pent =  SP_skill('Pentuple attack',  0, sph_enem_ST, spi_atk_multi, 'atk', 5, 0, 6, 4, 4, None)
sp_s_atk_quad =  SP_skill('Quadruple attack', 0, sph_enem_ST, spi_atk_multi, 'atk', 4, 0, 5, 3, 3, sp_s_atk_pent)
sp_s_atk_tripl = SP_skill('Triple attack',    0, sph_enem_ST, spi_atk_multi, 'atk', 3, 0, 5, 2, 2, sp_s_atk_quad)
sp_s_atk_doubl = SP_skill('Double attack',    0, sph_enem_ST, spi_atk_multi, 'atk', 2, 0, 3, 1, 1, sp_s_atk_tripl)

sp_s_atk_v4 = SP_skill('Strong attack v4', 0, sph_enem_ST, spi_atk_strong, 'atk', 2.0, 0, 6, 4, 4, None)
sp_s_atk_v3 = SP_skill('Strong attack v3', 0, sph_enem_ST, spi_atk_strong, 'atk', 1.7, 0, 5, 3, 3, sp_s_atk_v4)
sp_s_atk_v2 = SP_skill('Strong attack v2', 0, sph_enem_ST, spi_atk_strong, 'atk', 1.5, 0, 4, 2, 2, sp_s_atk_v3)
sp_s_atk_v1 = SP_skill('Strong attack v1', 0, sph_enem_ST, spi_atk_strong, 'atk', 1.3, 0, 3, 2, 1, sp_s_atk_v2)

sp_s_atk_arm_v4 = SP_skill('Armor piercing attack v4', 0, sph_enem_ST, spi_atk_arm, 'atk', 5, 0, 5, 2, 4, None)
sp_s_atk_arm_v3 = SP_skill('Armor piercing attack v3', 0, sph_enem_ST, spi_atk_arm, 'atk', 4, 0, 4, 2, 3, sp_s_atk_arm_v4)
sp_s_atk_arm_v2 = SP_skill('Armor piercing attack v2', 0, sph_enem_ST, spi_atk_arm, 'atk', 3, 0, 4, 2, 2, sp_s_atk_arm_v3)
sp_s_atk_arm_v1 = SP_skill('Armor piercing attack v1', 0, sph_enem_ST, spi_atk_arm, 'atk', 2, 0, 3, 2, 1, sp_s_atk_arm_v2)

sp_s_mana_maso_v1 = SP_skill('Mana masochism v1', 0, sph_self, spi_hp_to_mana, 'conv', 20, 1.0, 0, 0, 1, None)

#---passive attacks(if you can call them attacks)---
                  #SP_passive(name, cost, sph, spi, fnc_name, value_1, value_2, level, upgrade)
sp_p_heal_AT_v4 = SP_passive('Passive heal allies v4', 0, sph_ally_AT, spi_heal, 'heal', 4, 0, 4, None)
sp_p_heal_AT_v3 = SP_passive('Passive heal allies v3', 0, sph_ally_AT, spi_heal, 'heal', 3, 0, 3, sp_p_heal_AT_v4)
sp_p_heal_AT_v2 = SP_passive('Passive heal allies v2', 0, sph_ally_AT, spi_heal, 'heal', 2, 0, 2, sp_p_heal_AT_v3)
sp_p_heal_AT_v1 = SP_passive('Passive heal allies v1', 0, sph_ally_AT, spi_heal, 'heal', 1, 0, 1, sp_p_heal_AT_v2)

sp_p_dmg_AT_v4 = SP_passive('Passive damage enemies v4', 0, sph_enem_AT, spi_dmg, 'dmg', 4, 0, 4, None)
sp_p_dmg_AT_v3 = SP_passive('Passive damage enemies v3', 0, sph_enem_AT, spi_dmg, 'dmg', 3, 0, 3, sp_p_dmg_AT_v4)
sp_p_dmg_AT_v2 = SP_passive('Passive damage enemies v2', 0, sph_enem_AT, spi_dmg, 'dmg', 2, 0, 2, sp_p_dmg_AT_v3)
sp_p_dmg_AT_v1 = SP_passive('Passive damage enemies v1', 0, sph_enem_AT, spi_dmg, 'dmg', 1, 0, 1, sp_p_dmg_AT_v2)
...
def_potion = Item('Defence potion', 100, 'def up', i_def_up, 2, 6)
def_potionv2 = Item('Defence potion v2', 180, 'def up', i_def_up, 4, 4)

frail_potion = Item('Frailing potion', 120, 'def down', i_def_down, 1, 8)
frail_potionv2 = Item('Frailing potion', 190, 'def down', i_def_down, 2, 6)
...
test_boi_5 = Char('_test_boi_ 5', 100, 10, 0, mana_max=1000, mana_gain=100,
