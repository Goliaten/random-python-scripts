...
def pause(x=0):   #x=1 if for secret debug menu
    o = input("Press anything to continue ")      #function for pausing
    if x == 1:
        return o
        ...
        if len(x) > width:
            for z in range(width, -1, -1):  #spliting line
                ...
                if v == ' ':              #looking for closest space from the (length) character to first (i.e. looking for ' ' for nice split)
                    sep = z if z != 0 else 1    #+1          #separator #if separator is at 0 index, infinite loop wil be created
                    break
...
#             print('new_key', new_key)
            menus[new_key] = []
#         print('menus', menus)
#         print(f"{len(menus['menu1'])} / {length} ={int(len(menus['menu1']) / length)}")
#     pause()
...
temp_menu = [f'Initialising Game v{version}',
             ...
             '-currently drop tables are being introduced',
...
def stat_print(c=0):
    ...
    #d=0 info will be displayed on screen, d=1 info will be returned
    ret_list = []         #return list        #d is for printing outcome to list
    
    max_len = 0   #maximal length of name

    if c == 0:           #for making dynamic stat display(available length depends on length of character name)
        for x in team_1:
...
def team_target(c, sec_menu=[]):        #used for choosing target  #(c=1 target is team_1 c=2 target is team_2)
    ...
    sec_menu_bac = [z for z in sec_menu]
    z = 0
    if c == 1:               #target is team_1
        while True:
            sec_menu = [z for z in sec_menu_bac]
            y, z = -1, -1
            for x in team_1:
                y += 1
...
#         exp_gain = exp_gain if exp_gain else 0                     #exp gained
        ...
        if g_gain == 0:                                          #if gold is not specified
            for x in team_2:                              #get gold gained from monster gold_drop
                g_gain += x.g_drop
            temp_menu.append('You gained ' + str(g_gain) + ' gold')
            gold += g_gain     #add gained gold
            
        else:                                     #if gold was specified
            temp_menu = [
            'You won the fight',
            'You gained ' + str(g_gain) + ' gold'
            ]
        ...
        if exp_gain == 0:
            ...
            for x in team_1:
                ...
                temp_menu.append(x.name + ' gained ' + str(exp_gain) + ' experience points')
        ...
        if def_drop:
            ...
            for y in def_drop:                       #adding comment to temp_menu
                comm += f'{y.name}, '
            temp_menu.append(comm + 'which you gather.')
            
        else:
            comm = f'{"Monsters" if len(team_2)>1 else "Monster"} belched out: '
            for x in team_2:                        #going over each definitive drop table of the enemies
                held_mater.append(x.def_drop)
                for y in x.def_drop:
                    comm += f'{y.name}, '                            #also comment adding
            temp_menu.append(comm + 'which you gather.')
        ...
        if prb_drop:
            ...
            if temp_drop:                            #if probable drop was indeed gained
                ...
                for y in temp_drop:
                    comm += f'{y.name}, '                    #appropriate comment is added
                temp_menu.append(comm + 'behind.')
...
def revival(hp, c=0):  #hp for health after revivalion, c=0 for ally ress c=1 for enem ress
    global k
    while True:
        temp_menu = ['Turn: ' + str(turn), ''] + stat_print(0)
        y = -1    #for indexes
...
def spi_dmg(target, q):   #(target=target from sph, q=value from sp_attack itself)  (q is damageto be dealt)
...
#         print(self.name, self.cost, self.attrib, self.attrib_lvl)
...
met_ingot =  Mater('Metal nugget', 0, 'metal', 'ingot',  1)
wood_plank = Mater('Wooden plank', 0, 'wood',  1)

held_mater = []   #held/owned materials

global_mater = [mat1, mat2, mat3, imp_horn, imp_heart, imp_soul, met_nugget, met_ingot, wood_plank]

dev_global_mater = []
for x in global_mater:
    dev_global_mater.append(x)
all_id_class += dev_global_mater
del dev_global_mater
...
#if there is no tree, there will be no progression(sp_atk are set in stone. nothing will change them)
...
# hero =     Char('Hero',     70,  10, 1, 100, defence=1,
...
# tree_step = [3, 5, 10, 12, 15]  <-- describes, at which levels sp_atk gain occur
#  it should be as long, as the longest tree (aka magic_tree) -1 (ommit first stage)
...
#     print(x.name)
    if x.name in ['_test_boi_ 1', '_test_boi_ 2']:
        continue
...
#--------------------imps---------------------
imp_1 = Char('Lesser Imp',  40,  4,  2, 0, 30,  10,  imp_def_drop_low, imp_prb_drop_low,  0, 0, 0, 'Subdemon', imp_bas_wep, imp_bas_arm)
imp_2 = Char('Imp Servant', 40,  4,  2, 0, 45,  20,  imp_def_drop_low, imp_prb_drop_low,  0, 0, 0, 'Subdemon', imp_lig_wep, imp_lig_arm, AI=AI_servant)
imp_3 = Char('Imp',         60,  7,  1, 0, 75,  35,  imp_def_drop_med, imp_prb_drop_med,  0, 0, 0, 'Subdemon', imp_lig_wep, imp_lig_arm, AI=AI_weakfirst)
imp_4 = Char('Imp Soldier', 60,  7,  1, 0, 120, 45,  imp_def_drop_med, imp_prb_drop_med,  0, 0, 0, 'Subdemon', imp_med_wep, imp_med_arm, AI=AI_servant)
...
allies = [hero, mage]
# allies = [hero, champion, mage, test_boi_1, test_boi_2]
# allies = [test_boi_1, test_boi_2]
...
#after one package of data has been read, '---\n' is used to mark end of data pack
            ...
            if a == 4:
                try:
                    ...
                    menu_print(['File ' + savename + ' is outdated'], 1)
                    pause()
                    
                except:
                    menu_print(['File ' + savename + ' used as save does not exist'], 1)
                    pause()
