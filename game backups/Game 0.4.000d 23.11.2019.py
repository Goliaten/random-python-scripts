...
def spf_heal_all(z, target):
    if target == 'team 1':  #converting target
        target = team_1
    else:
        target = team_2
        
    for y in target:
        if y.hp == 0:         #dont ressurect
            continue
        
        y.hp += z       #actual healing

        if y.hp > y.hp_max:                     #prevent overheal
            y.hp = y.hp_max
        y.stat_set()
        
    if target == team_1:           #for return
        target = 'team 1'
    else:
        target = 'team 2'
    return f'{x.name} used mass healing magic and healed {z} health to {target}'         #see item>use_item

def spf_dmg_all(z, target):
    if target == 'team 1':     #converting target
        target = team_1
    else:
        target = team_2
        
    for y in target:          #dealing damage
        x.take_dmg(z)
    ...
    if target == team_1:         #for return
        ...
        target = 'team_2'
    return f'{x.name} used area damage and inflicted {z} damage to {target}'         #see item>use_item
...
def spf_str_atk(z, target):   #strong attack
    global p
    if target == 'team 1':                                      #setting target
        team_target(1, ['Choose target for double attack'])
        target = team_1
    elif target == 'team 2':
        team_target(2, ['Choose target for double attack'])
...
def spf_pass_heal(z, target, name):
    if target == 'team 1':
        target = team_1
    elif target == 'team 2':
        target = team_2
    ...
    if target == team_1:
        ...
        target = 'team 2'
    return f'Everyone from {target} has been healed for {z} health by {name}.'

def spf_pass_dmg(z, target, name):
    if target == 'team 1':
        target = team_1
    elif target == 'team 2':
        target = team_2
    ...
    if target == team_1:
        ...
        target = 'team 2'
    return f'Everyone from {target} took {z} damage from {name}.'
...
class sp_passive(sp_atk):
    ...
    def activate(self):
        t_comm = self.fnc(self.value, self.target, self.name)
        return t_comm
...
global_weapons = [bat, sword, hammer, heal_sword, creator_touch]   #for shop display
                        ...
                        while True:
                            for x in team_1:
                                ...
                                if x.passive:
                                    ...
                                    for y in x.passive:
                                        temp_menu.append(f'  {y.name} for {y.value} target:{y.target}')
